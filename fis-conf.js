...
fis.config.set('name', 'CannonHero');
//把前面的配置都写在这里统一管理
//项目中就不用再写了
fis.config.set('roadmap.path', [
  {
    reg : /.*\.md|.*\.inline\.png|.*\.tpl|map.json|conf\/.*|test\/.*|design\/.*|templates\/.*|.*\.inline\.js/,
    release : false  //不发布
  },
  {
    ...
    settings : {
        ...
        postpackager : {
            ...
            simple: {
              ...
              autoReflow: true,
              // headTag: '<!--HEAD_END-->',
              output: 'CannonHero/auto_combine_${hash}'
            },
            inline: {
              exclude : [
                /.*common\.js/i
